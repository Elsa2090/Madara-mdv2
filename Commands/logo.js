...
zokou({ nomCom: "hacker",
    ...
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("*Bryant tech is on it*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "\t *Logo by MADARA_MD-V2*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        const noArgMsg = `*_EXEMPLE *:  ${prefixe}dragonball Bryant tech`;
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: " \t *Logo by MADARA_MD-V2*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto MADARA_MD-V2);
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t *Logo by MADARA_MD-V2*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t *Logo by MADARA_MD-V2*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
           repondre(`*exemple :* ${prefixe}didong Bryant tech`)
            return ;
        }
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by *MADARA_MD-V2*' }, { quoted: ms });
...
zokou({nomCom: "greenneon", categorie: "Logo", reaction: "🟢"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/green-neon-text-effect-874.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by *Bryant tech*' }, { quoted: ms });
    })
    .catch(console.error);
});
  ...
  {nomCom:"boom",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom MADARA_MD-V2`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by MADARA_MD-V2*"})
         
        // await zk.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by BRYANT TECH*"})
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold MADARA_MD-V2");return;
      }
...
zokou({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena MADARA_MD-V2");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent MADARA_MD-V2");return;
      }
...
zokou({nomCom:"child",categorie:"Logo",reaction:"😇"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"child MADARA_MD-V2");return;
      }
...
zokou({nomCom:"typo",categorie:"Logo",reaction:"😇"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"typo MADARA_MD-V2");return;
      }
    ...
    var img = await mumaker.ephoto(lien,arg.join(' '));
   repondre("processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:" *Logo by BRYANT_TECH*"},{quoted:ms})
  }catch(e){repondre(e)}
})
...
zokou({nomCom:"light",categorie:"Logo",reaction:"🌠"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"light MADARA_MD-V2");return;
      }
...
zokou({nomCom:"steel",categorie:"Logo",reaction:"🌠"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"steel MADARA_MD-V2");return;
      }
