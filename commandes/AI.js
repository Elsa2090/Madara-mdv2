...
zokou({nomCom:"bot",reaction:"🎁",categorie:"IA"},async(dest,zk,commandeOptions)=>{
  ...
  const {repondre,ms,arg}=commandeOptions;
    ...
    if(!arg || !arg[0])
    {return repondre("yes this is madara_md-v2 what do you want.")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*POWERED BY BRYANT TECH*';
    ...
    zokou({ nomCom: "madara", reaction: "🧑‍🏫", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
